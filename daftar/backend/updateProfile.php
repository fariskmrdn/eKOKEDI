...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['token']) && verifyCSRFToken($_POST['token'])) {
        ...
        $occ_state = filter_input(INPUT_POST, 'occ_states', FILTER_SANITIZE_SPECIAL_CHARS);
        ...
        if (
            ...
            try {
                ...
                $updateParentsProfile->execute([
                    ...
                    $occ_mother, $occ_mother_notel, $occ_mother_job, $occ_mother_salary, $occ_mother_dependants, $occ_mother_payslip, 
                    $occ_address, $occ_state, $student['occ_id']
                ]);


                if ($updateParentsProfile) {
                    $sendApplication = $pdo->prepare("INSERT INTO applications (occ_id, application_status) VALUES (?,?)");
                    $sendApplication->execute([$student['occ_id'], '0']);
                    ...
                    $mail->addAddress('ekokedikvks@gmail.com');
                    $mail->isHTML(true);
                    $mail->Subject = 'Notifikasi: Permohonan Baru Penempatan Kolej Kediaman Sesi 2024';
                    $mail->Body = 'Salam sejahtera, terdapat permohonan baru bagi penempatan kolej kediaman melalui eKOKEDI. Perhatian dan kerjasama anda amat dihargai. Sekian.';
                    $mail->send();
